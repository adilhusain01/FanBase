...
export function FanBaseDashboard({ match }: { match: Match | null }) {
  ...
  return <main className="shell">
    ...
    <section id="apply" className="application-section">
      <div className="section-heading"><h2>Enter without the panic refresh.</h2><p>Fixtures refresh from football-data.org, with FIFA’s official calendar as fallback. FanBase is a prototype, not an official FIFA ticket seller.</p></div>
      ...
      <div className="apply-panel"><div><h3>{application ? "You’re eligible for the draw." : "Ready when you are."}</h3><p>{application ? application.explanation : "Connect your Injective testnet wallet, then create a verifiable application. Nothing is ranked in a hidden queue."}</p></div>{application ? <div className="score-block"><span>Fan signal score</span><strong>{application.score}</strong><small>×{application.weight.toFixed(2)} draw weight</small></div> : <button className="primary" disabled={!isConnected || chainId !== injectiveTestnet.id || !match || busy} onClick={apply}>{busy ? <LoaderCircle className="spin" size={17} /> : "Enter draw"}<ChevronRight size={17} /></button>}</div>
